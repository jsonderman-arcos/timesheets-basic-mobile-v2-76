-- First, let's create a sample user for the demo
...
-- Add a more permissive RLS policy for development/demo purposes
CREATE POLICY "Allow demo user to create time entries" 
ON time_entries 
FOR INSERT 
WITH CHECK (
  -- Allow if no authentication (for demo) OR if authenticated user owns the crew
  auth.uid() IS NULL OR 
  crew_id IN (
    SELECT crews.id
    FROM crews
